...
# Description: Implements the permanent email deletion logic.
...
log = logging.getLogger("gmail_cleaner.delete")
...
async def run_delete(
    ...
    async def emit(event_type: str, data: dict) -> None:
        log.debug("[delete SSE] %s: %s", event_type, data)
    ...
    try:
        ...
        if dry_run:
            store_result_fn(
                {"deleted_count": total, "sender": sender_email, "dry_run": True}
            )
            await emit(
                "complete",
                {"deleted_count": total, "sender": sender_email, "dry_run": True},
